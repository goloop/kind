...
`kind` створено для тих, хто пише **парсери, декодери й біндери** - код, що бере
`string`, рядок таблиці, змінну оточення чи значення конфігу й вкладає його в
довільний Go-тип. Ця робота зазвичай виливається в купу ручного `reflect`: «це
int? вказівник на структуру? чи реалізує воно `encoding.TextUnmarshaler` на
pointer-receiver? який тип елемента за цими двома слайсами?»

`kind` відповідає на ці питання одним кешованим дескриптором і пласким
словником предикатів, тож парсер можна випустити, не живучи всередині `reflect`:
...
Це **специфічний** інструмент, а не універсальний фреймворк: якщо ти ніколи не
тягнешся по `reflect`, тобі `kind` не потрібен. Але коли тягнешся - він прибирає
нудну, схильну до помилок половину роботи й кешує аналіз типу, тож гарячий цикл
парсингу платить за нього один раз на тип.
